...
export default async (message: Message | ChatInputCommandInteraction, client: Client) => {
    const embed = new EmbedBuilder()
        ...
        .addFields(
            ...
            { name: 'Giải trí với bò', value: '``cow (xem ảnh bò - tốn ' + `${config.imagePrice}cc` + ')`` ,``cat (xem ảnh mòe)``,``happy (đọc truyện cười - tốn ' + `${config.getJokePrice}cc` + ')``, ``joke noidung (đóng góp truyện cười, nếu được duyệt bạn sẽ được' + `${config.addJokePrice}cc` + ')``,``chanle (random có thưởng - chỉ dùng slash)`` , ``s (vũ trụ có bò)``' },
